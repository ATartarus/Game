...
Map::Map(const char* map)
{
	...
	backgroundTiles.matrix = nullptr;
	objects		           = nullptr;
...
{
	if (backgroundTiles.matrix == nullptr) return;
	for (auto& vec : *backgroundTiles.matrix)
	{
		for (auto t : vec) delete t;
	}
	delete backgroundTiles.matrix;
}
...
void Map::loadMap(const char* map)
{
	...
	while (layer)
	{
		...
		else if (layerName == "layer")
		{
			...
			{
				deleteBackgroundTiles();
				parseColor(backgroundTiles.tintColor, layer->Attribute("tintcolor"));

				backgroundTiles.matrix = new std::vector<std::vector<sf::Sprite*>>(layer->FindAttribute("height")->IntValue());
				for (auto& vec : *backgroundTiles.matrix) vec = std::vector<sf::Sprite*>(layer->FindAttribute("width")->IntValue());
				loadTiles(csv, true);
			}
			else
			{
	...
	for (int i = 0; i < csvMap.size(); i++) {
		for (int j = 0; j < csvMap[0].size(); j++) {
			...
			if (background)
			{
				...
				if (backgroundTiles.tintColor != sf::Color()) tmp->setColor(backgroundTiles.tintColor);
				(*backgroundTiles.matrix)[i][j] = tmp;
			}
			else
			{
...
void Map::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	for (int i = Y1; i < Y2; i++) {
		for (int j = X1; j < X2; j++) {
			if (backgroundTiles.matrix && (*backgroundTiles.matrix)[i][j])
			{
				target.draw(*(*backgroundTiles.matrix)[i][j]);
			}
